...
**Structured, not a chaotic dump.** Entries are dense and organized — English, dated, thematic. Organize by theme in `process-docs/<area>/` subfolders; file naming inside is free (date-based, purpose-based, both OK). A one-off may be a single loose top-level `.md`; a theme with several entries gets a subfolder.

**Evidence stays discoverable.** A measurement that justifies a code value (why `threshold=0.48`) lives in the process-docs entry plus the dev/ report it came from — cite the dev/ script, the report-MD, and the dataset/sample size. The full report stays in `dev/<area>/`, committed. The resume path is RAG over `process-docs` + reading the code; there is no crystallized-conclusion file to consult.
