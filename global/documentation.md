...
Everything you read or produce as ARTIFACT — code, DOCS.md, rules/, decisions/, code-comments — is machine-readable and token-dense.
...
**New-area flow:** create `decisions/<area>.md` (may stay empty at first) → create the `decisions/OldThemes/<area>/` folder → once dev work begins, create `dev/<area>/`. All three share the area name.
...
Decision records — FINAL STATE ONLY. Each file documents the current production choice (IST), the evidence backing it (Evidenz), and the open questions that remain (Offene Fragen). What SHOULD change is NOT part of a decision file — a target/recommendation belongs in an issue, not in the documentation. The process that led to the current choice — alternatives evaluated, superseded values, iteration history — lives in `decisions/OldThemes/<area>/`, NOT here. A decision file is the crystallized conclusion; OldThemes is the working memory that produced it.
...
#### Evidenz
The data that BACKS the IST — measurements from dev/ scripts, external research, benchmarks. For an external source: its actual content/finding plus a short reference to where it came from, not a bare link. For an internal measurement: the key result plus a reference to the dev/ report that produced it. Whenever evidence for an IST exists, it MUST be listed here.

**Internal eval results — cite script AND report-MD AND dataset.** When a decision is justified by a measurement run, Evidenz carries the key result and cites:
...
#### Quellen
The COMPLETE list of all external sources consulted — RAG Collection references, papers, URLs. External sources only; internal eval reports belong in Evidenz, not here. Division of labor: Evidenz carries each source's content with a short reference, Quellen is the full list behind those references.
...
| File | Content | Scope |
|---|---|---|
| `decisions/<area>.md` | Current prod IST + Evidenz that backs it + open questions | Final state only |
...
**Values in OldThemes ≠ current status quo.** A number measured in iteration 3 is preserved in OldThemes as historical record, not as IST. Only what is currently in production code lives in `decisions/<area>.md`.

**The decision derives from OldThemes progress.** A decision file crystallizes the conclusion of the process documented in the matching OldThemes folder. While the process there has not converged, the open points live in the decision file's Offene Fragen. Once it converges, the chosen value lands in IST and `decisions/<area>.md` cites the matching OldThemes file in Evidenz.

**Demotion rule.** When new evidence supersedes an older measurement (new collection, new methodology, new code), the older measurement moves from `decisions/<area>.md` Evidenz to `decisions/OldThemes/<area>/`. decisions/ never carries stale evidence; OldThemes preserves it for attribution and traceability.
