...
**Self-contained — NO references to other files.** A process-docs entry references NO other documentation or report file — not another process-docs entry, not a dev/ report, not a DOCS.md, not a former decision doc. Cross-file path links rot: files move, get renamed, get deleted, and the link becomes a dead pointer — this happens constantly and is untenable. Thematic grouping is carried by the FOLDER the entry lives in; related work is found by RAG over `process-docs` + browsing the folder, never by a hardcoded path. State a measurement's result INLINE (the number, the dataset size, the finding); do not link the dev/ report that produced it. Naming a source-code file or symbol as descriptive context ("the threshold lives in `scrape_url.py`") is fine — the code is the ground truth and is not a rot-prone doc link.

**Evidence stays discoverable.** The resume path is RAG over `process-docs` + reading the code; there is no crystallized-conclusion file to consult. A measurement that justifies a code value (why `threshold=0.48`) is stated inline in the process-docs entry, with the key result and dataset/sample size in the prose itself. The full dev/ report stays in `dev/<area>/`, committed, and is found by browsing dev/ — not by a link from the entry.
